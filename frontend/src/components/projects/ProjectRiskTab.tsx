...
const LEVEL_STYLE: Record<Level, { bg: string; text: string; border: string; label: string; icon: any }> = {
  green:  { bg: 'bg-green-50 dark:bg-green-900/10', text: 'text-green-700 dark:text-green-400',
            border: 'border-green-200 dark:border-green-900/50', label: 'Низкий риск', icon: CheckCircle2 },
  yellow: { bg: 'bg-surface-50 dark:bg-surface-900/10',     text: 'text-surface-700 dark:text-surface-400',
            border: 'border-surface-200 dark:border-surface-900/50',     label: 'Средний риск', icon: AlertCircle },
    ...
    <div className="space-y-6">
      ...
      {triggered.length > 0 && (
        <section>
          <h3 className="font-semibold text-sm mb-3 flex items-center gap-2">
            <AlertTriangle className="text-surface-500" size={16} />
            Активные триггеры ({triggered.length})
          </h3>
          <ul className="space-y-2">
            {triggered.map(f => (
              <li key={f.key} className="flex items-center gap-3 px-4 py-3 rounded-lg border border-surface-200 dark:border-surface-900/50 bg-surface-50 dark:bg-surface-900/10">
                <span className="w-2 h-2 rounded-full bg-surface-500 shrink-0" />
      ...
      {projectType === 'SMM' && (
        <section>
          ...
          {pfLoading ? (
            <div className="flex items-center justify-center py-6"><Loader2 className="animate-spin text-surface-500" /></div>
...
function PlanFactBar({ row }: { row: PlanFactRow }) {
  ...
  const barColor = row.overuse > 0
    ? 'bg-red-500'
    : fillPct >= 100 ? 'bg-green-500'
    : fillPct >= 70 ? 'bg-surface-500'
    : 'bg-surface-500'
    ...
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3">
      ...
      <div className="flex items-center justify-between text-[11px] text-gray-500 mt-1.5">
        ...
        <div className="flex items-center gap-3">
          ...
          {row.underuse > 0 && (
            <span className="text-surface-600 dark:text-surface-400">Недотяг −{row.underuse}</span>
          )}
          {row.remaining > 0 && (
