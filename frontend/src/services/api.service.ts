...
export const projectsApi = {
  ...
  setSmmCycle: (id: string, data: { day?: number | null; normReels?: number | null; normPosts?: number | null; anchor?: string | null; storiesPerMonth?: number | null }) =>
  ...
  setSmmProfile: (id: string, data: { ownerName?: string | null; keyDate?: string | null; keyDateNote?: string | null; collabSince?: string | null; preferences?: string | null; smmSpecialistIds?: string[]; videographerIds?: string[]; videoEditorIds?: string[]; designerIds?: string[]; metrics?: { ym: string; subs?: number | null; reach?: number | null; eng?: number | null; leads?: number | null }[] }) =>
...
export const contentPlanApi = {
  ...
  // Календарь производства за диапазон дат: публикации + съёмки.
  // segment: 'smm' (по умолчанию) — раздел СММ, 'dev' — раздел «Разработка».
...
export const financeApi = {
  ...
  addEmployeeDeduction: (id: string, data: { kind: 'fine' | 'vacation' | 'bonus'; ym: string; amount: number; date?: string; dateFrom?: string; dateTo?: string; note?: string }) => api.post(`/finance/employees/${id}/deduction`, data).then(r => r.data),
  updateEmployeeDeduction: (id: string, entryId: string, data: { kind: 'fine' | 'vacation' | 'bonus'; ym: string; note?: string }) => api.patch(`/finance/employees/${id}/deduction/${entryId}`, data).then(r => r.data),
  removeEmployeeDeduction: (id: string, entryId: string, params: { kind: 'fine' | 'vacation' | 'bonus'; ym: string }) => api.delete(`/finance/employees/${id}/deduction/${entryId}`, { params }).then(r => r.data),
