...
// Линии-связки убраны: связь «съёмка → её выход» теперь пишется числом прямо на
// карточке подготовки. Для этого карточке нужен её родитель — ищем по itemId.
type PubState = { byItem: Map<string, Ev>; today: string; soon: string }
const PubCtx = createContext<PubState>({ byItem: new Map(), today: '', soon: '' })
...
export default function SmmPage({ embeddedProjectId }: { embeddedProjectId?: string } = {}) {
  ...
  const pubByItem = useMemo(() => {
    ...
    for (const e of allEvents) if (e.kind === 'publication' && e.itemId) m.set(e.itemId, e)
    return m
  }, [allEvents])
  const soonIso = useMemo(() => iso(addDays(new Date(), 1)), [])
                    ...
                    {timedFor(d, h).map(s => {
                      ...
                      return (
                        <div key={s.id} data-ev={s.id} data-proj={s.projectId} data-reel={s.reelId ? `item:${s.reelId}` : undefined}
                          ...
                          style={{ top: (mm / 60) * HOUR_PX + 1, height: Math.max(20, (dur / 60) * HOUR_PX - 2), left: `calc(${leftPct}% + 1px)`, width: `calc(${widthPct}% - 2px)`, opacity: (sel.ids.size > 0 && !sel.ids.has(s.id)) ? 0.24 : 1, ...projFill(s.projectId), boxShadow: s.reelId ? (sel.ids.has(s.id) ? '0 2px 8px rgba(0,0,0,.5)' : '0 1px 3px rgba(0,0,0,.35)') : (sel.ids.has(s.id) ? `inset 0 0 0 2px ${projColor(s.projectId)}, 0 2px 8px rgba(0,0,0,.5)` : `inset 0 0 0 1px color-mix(in srgb, ${projColor(s.projectId)} 60%, transparent), 0 1px 3px rgba(0,0,0,.35)`), ...(s.reelId ? { outline: `${sel.ids.has(s.id) ? 2.5 : 2}px dashed ${projColor(s.projectId)}`, outlineOffset: '-2px' } : {}) }}>
                        ...
                        {timedFor(d, h).map(s => {
                          ...
                          return (
                            <div key={s.id} data-ev={s.id} data-proj={s.projectId} data-reel={s.reelId ? `item:${s.reelId}` : undefined}
                              ...
                              onDragStart={(ev) => { grabOffsetRef.current = (ev.nativeEvent as DragEvent).offsetY || 0; onDragStart(s) }}
                              className="absolute rounded-md px-1.5 py-1 overflow-hidden cursor-grab active:cursor-grabbing z-[2] transition hover:brightness-110"
                              style={{ top: (mm / 60) * HOUR_PX + 1, height: Math.max(20, (dur / 60) * HOUR_PX - 2), left: `calc(${leftPct}% + 1px)`, width: `calc(${widthPct}% - 2px)`, opacity: dim ? 0.24 : 1, ...projFill(s.projectId), boxShadow: s.reelId ? (on ? '0 2px 8px rgba(0,0,0,.5)' : '0 1px 3px rgba(0,0,0,.35)') : (on ? `inset 0 0 0 2px ${projColor(s.projectId)}, 0 2px 8px rgba(0,0,0,.5)` : `inset 0 0 0 1px color-mix(in srgb, ${projColor(s.projectId)} 60%, transparent), 0 1px 3px rgba(0,0,0,.35)`), ...(s.reelId ? { outline: `${on ? 2.5 : 2}px dashed ${projColor(s.projectId)}`, outlineOffset: '-2px' } : {}) }}>
                              <div className="flex items-center gap-1 text-[12px] font-medium leading-tight"><Camera size={11} className="shrink-0" /><span className="truncate">{s.projectName || s.title}</span></div>
...
function EventChip({ e, onOpen, onDragStart }: { e: Ev; onOpen?: (e: Ev) => void; onDragStart?: (e: Ev) => void }) {
  ...
  const showOut = e.kind === 'shoot' && !!e.reelId
  const parentPub = showOut && e.reelId ? pub.byItem.get(e.reelId) : undefined
  ...
  return (
    <span data-ev={e.id} data-proj={e.projectId} data-reel={e.reelId ? `item:${e.reelId}` : undefined}
      ...
      {showOut && (
        <span title={parentDone ? 'Публикация уже вышла' : outDate ? `Выход ${outDate}` : 'Выход ещё не запланирован'}
          className={'ml-auto shrink-0 inline-flex items-center rounded px-1 text-[8.5px] font-extrabold tabular-nums border '
            + (parentDone
              ? 'text-emerald-600 dark:text-emerald-400 border-emerald-400/60 bg-emerald-500/10'
              : !outDate
                ? 'text-gray-400 border-gray-400/50 bg-gray-400/10'
                : outHot
                  ? 'text-red-500 dark:text-red-400 border-red-400/60 bg-red-500/10'
                  : 'text-primary-600 dark:text-primary-400 border-primary-400/60 bg-primary-500/10')}>
          {parentDone ? <Check size={9} strokeWidth={3} /> : outDate ? Number(outDate.slice(8, 10)) : '—'}
        </span>
      )}
    </span>
  )
