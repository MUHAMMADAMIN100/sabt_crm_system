...
export class ContentPlanService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => TelegramService)) private telegram: TelegramService,
  ...
  /** Утренняя сводка производству (9:00 по Душанбе): каждому исполнителю —
   *  что у него сегодня. Именно это и значит «задачи приходят сами»: человек
   *  не заходит проверять кабинет, а получает список в Telegram.
   *
   *  Шлём только тем, у кого на сегодня есть незакрытые карточки: пустых
   *  «сегодня задач нет» не рассылаем, иначе уведомления перестают читать. */
  ...
  async notifyProductionMorning() {
    ...
    const rows: any[] = await this.repo.manager.query(
      ...
      [today],
    ).catch((e: any) => { this.logger.warn(`notifyProductionMorning query failed: ${e?.message || e}`); return []; });
    ...
    for (const [userId, list] of byUser) {
      ...
      try {
        ...
        this.logger.warn(`Telegram digest failed for ${userId}: ${(e as Error).message}`);
      }
      try {
        ...
        this.logger.warn(`Notification digest failed for ${userId}: ${(e as Error).message}`);
      }
    }
