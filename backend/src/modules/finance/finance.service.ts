...
/** Таджикские буквы к русским: у нас одного человека пишут и «Муҳаммад», и
 *  «Мухаммад», а в ведомости и в CRM — по-разному. Раньше эти буквы вообще
 *  выбрасывались как «не буквы»: «Муҳаммад» превращалось в «му аммад», имя
 *  распадалось на два слова, и строка ведомости не находилась — человек
 *  открывал профиль и не видел своей зарплаты. */
const TJ_TO_RU: Record<string, string> = {
  'ғ': 'г', 'ӣ': 'и', 'қ': 'к', 'ӯ': 'у', 'ҳ': 'х', 'ҷ': 'ч',
};
/** Слова имени: без регистра, без «ё», таджикские буквы приведены к русским,
 *  без лишних знаков и пробелов. */
const nameWords = (s?: string | null): string[] =>
  String(s || '')
    .toLowerCase()
    .replace(/ё/g, 'е')
    .replace(/[ғӣқӯҳҷ]/g, ch => TJ_TO_RU[ch] || ch)
